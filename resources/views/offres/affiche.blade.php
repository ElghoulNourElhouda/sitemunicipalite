...
@if(!Auth::guest())
@if (App\Views::where('users_id',auth()->id())->where('offre_id',$offre->id)->count() == 0)
@if($offre->users_id != auth()->id())
<?php  App\Views::create(['users_id' => auth()->id(), 'offre_id' => $offre->id]); ?>
@endif
@endif
@endif
    ...
    <div class="col-md-8 col-md-offset-2" style="background-color: rgba(240, 255, 240, 0.8); margin-bottom: 10px;">
     <div id="ToPrint" style="padding-top: 10px;">
      <p align="center">
        ...
        <img class="img-responsive img-fluid" id="myImg" src="../storage/articles_images/{{ $offre->picture }} " alt="Article image" style="border: solid #E6E6FA 1px; max-width: 50%; max-height: 25%;" >

        <!--  <a class="btn btn-link" data-toggle="modal" href="../storage/articles_images/{{ $offre->picture }}" download> download image</a> -->
        @else
      ...
      <h2>{{$offre->work}} 
        ...
        <ul>
          <li><strong>Niveau</strong> :<b style="color:#08123a"><u> {{ $offre->level }} </u></b></li>

          <li><strong>Formation</strong> : {{ $offre->formation }}</li>

          <li><strong>lieu</strong> : {{ $offre->location }}</li>
      ...
      @if( $offre->blocked )
      <a href="" class="btn btn-primary btn-sm" disabled> Diposer documents </a>
      <div class="p-3 mb-2 bg-danger">
        <div class="media" style="padding-top: 10px; padding-bottom: 10px; padding-left: 10px">
          <div class="media-left media-middle">
            <i class="fa fa-exclamation-triangle pull-left"></i>
          </div>
          <div class="media-body"><b class="text text-danger"> cette offre est bloqué par l'administrateur </b>
          </div>
        </div>
      </div>
      @else
      <a href="{{ url('condidature/'.$offre->id) }}" class="btn btn-primary btn-sm"> Diposer documents </a>
      @endif
      @endif
      ...
      <div class="p-3 mb-2 bg-danger">
        <div class="media" style="padding-top: 10px; padding-bottom: 10px; padding-left: 10px">
          ...
          <div class="media-body">
            <strong class="text tex-warning"> <b> Veuillez se connecter pour pouvoir diposer à cette offre ! </b> <small> 
              <a href="{{ url('/login') }}">se connecter !</a></small></strong>
            </div>
          </div>
        </div>
        ...
        <!-- <div class="col-md-8 col-md-offset-2" style="margin-bottom: 10px; background-color: rgba(240, 255, 240, 0.8);">
        	<br>
        	{{ Form::open(array('url' => 'comment_task/' . $offre->id)) }}
               {{ csrf_field() }}
